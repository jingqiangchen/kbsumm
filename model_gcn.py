...
#
  ...
  def _add_entities2entities_loss_new(self, emb_entity_inputs, entities2entities):
    ...
    entities2entities2=tf.matmul(emb_entity_inputs, emb_entity_inputs, transpose_a=False, transpose_b=True)
    eye=tf.eye(n)
  ...
  def _add_entities2entities_loss_old(self, emb_entity_inputs, entities2entities):
  ...
  def _add_gcn_sota(self):
      ...
      self._sent_relas_DAD = tf.where(tf.greater(self._sent_relas_DAD, tf.zeros_like(self._sent_relas_DAD)), 
                                      tf.ones_like(self._sent_relas_DAD), tf.zeros_like(self._sent_relas_DAD))
      self._entities2entities_DAD = tf.where(tf.greater(self._entities2entities_DAD, tf.zeros_like(self._entities2entities_DAD)), 
                                             tf.ones_like(self._entities2entities_DAD), tf.zeros_like(self._entities2entities_DAD))
      self._entities2sents_DAD = tf.where(tf.greater(self._entities2sents_DAD, tf.zeros_like(self._entities2sents_DAD)), 
                                          tf.ones_like(self._entities2sents_DAD), tf.zeros_like(self._entities2sents_DAD))
      self._sents2entities_DAD = tf.where(tf.greater(self._sents2entities_DAD, tf.zeros_like(self._sents2entities_DAD)), 
                                          tf.ones_like(self._sents2entities_DAD), tf.zeros_like(self._sents2entities_DAD))
  ...
  def _add_kbsumm(self):
    ...
    if hps.use_entity_embedding:
        ...
        with tf.variable_scope('kbsumm_entity_embedding'): 
            if hps.entity_vocab_size // 10000 == 4:
                path_entity_vocab_embeddings=common.path_entity_vocab_embeddings_4
            elif hps.entity_vocab_size // 10000 == 5:
            ...
            elif hps.entity_vocab_size // 10000 == 30:
                path_entity_vocab_embeddings=common.path_entity_vocab_embeddings_30 
            elif hps.entity_vocab_size // 10000 == 50:
                path_entity_vocab_embeddings=common.path_entity_vocab_embeddings_50
    ...
    with tf.variable_scope('gcn'):
        if hps.gcn_method=="sota":
            sent_gcn_enc, entity_gcn_enc = self._add_gcn_sota()
        else:
      ...
      if hps.mode in ['train', 'eval']:
        ...
        with tf.variable_scope('loss'):
          ...
          if hps.lambda_ee_train>0 and hps.use_gcn_entity:
              if hps.en_loss_ver=="new":
                  ee_loss=self._add_entities2entities_loss_new(emb_entity_inputs, self._entities2entities)
              elif hps.en_loss_ver=="old":
                  ee_loss=self._add_entities2entities_loss_old(emb_entity_inputs, self._entities2entities)
