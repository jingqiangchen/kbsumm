...
tf.app.flags.DEFINE_string('en_loss_ver', 'new', '')
tf.app.flags.DEFINE_string('gcn_method', 'default', '')
...
def training_greedy(model,batch,sess,gen_decoder):
    ...
    for index in range(FLAGS.batch_size):
        ...
        #greedy_rewards.append(100 * rouge.get_scores(gen_outputs_greedy[index],  ref_summary)[0]['rouge-1']['f'])  
        try:
            sample_reward1 = common.evaluate(ref_summary, gen_outputs_sample[index])['rouge-1']['f'] #rouge.get_scores(gen_outputs_sample[index],  ref_summary)[0]['rouge-1']['f']
            ...
            sample_reward1 = common.evaluate(ref_summary, sample_summary)['rouge-1']['f'] #rouge.get_scores(sample_summary,  ref_summary)[0]['rouge-1']['f']
            #sample_reward2 = entity_reward(gen_outputs_sample[index], ref_entities)
            #sample_reward = (1-FLAGS.entity_lambda) * sample_reward1 + FLAGS.entity_lambda * sample_reward2
            sample_reward = sample_reward1
