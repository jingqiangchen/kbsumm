...
class BeamSearchDecoder(object):
  ...
  def write_results(self, reference_summary, decoded_words, attn_dist, ex_index, output_file, attn_file=None):
    ...
    try:
        ...
        scores = evaluator.get_scores([decoded_text], [reference_summary])
        #scores = rouge.get_scores([reference_summary], [decoded_text])
...
class GreedyDecoder(object):
  ...
  def write_results(self, reference_summary, decoded_text, ex_index, output_file):
    ...
    try:
        ...
        scores = evaluator.get_scores([decoded_text], [reference_summary])
        #scores = rouge.get_scores([reference_summary], [decoded_text])
        output_file.write("%f\t%f\t%f\n" % (scores['rouge-1']['r'], scores['rouge-1']['p'], scores['rouge-1']['f']))
        self.score_1[0]+=float(scores['rouge-1']['r'])
        self.score_1[1]+=float(scores['rouge-1']['p'])
        self.score_1[2]+=float(scores['rouge-1']['f'])
        output_file.write("%f\t%f\t%f\n" % (self.score_1[0]/ex_index, self.score_1[1]/ex_index, self.score_1[2]/ex_index))
          
        output_file.write("%f\t%f\t%f\n" % (scores['rouge-2']['r'], scores['rouge-2']['p'], scores['rouge-2']['f']))
        self.score_2[0]+=float(scores['rouge-2']['r'])
        self.score_2[1]+=float(scores['rouge-2']['p'])
        self.score_2[2]+=float(scores['rouge-2']['f'])
        output_file.write("%f\t%f\t%f\n" % (self.score_2[0]/ex_index, self.score_2[1]/ex_index, self.score_2[2]/ex_index))
          
        output_file.write("%f\t%f\t%f\n" % (scores['rouge-l']['r'], scores['rouge-l']['p'], scores['rouge-l']['f']))
        self.score_l[0]+=float(scores['rouge-l']['r'])
        self.score_l[1]+=float(scores['rouge-l']['p'])
        self.score_l[2]+=float(scores['rouge-l']['f'])
        output_file.write("%f\t%f\t%f\n" % (self.score_l[0]/ex_index, self.score_l[1]/ex_index, self.score_l[2]/ex_index))
  ...
  def write_results(self, sent_scores, entity_scores, data_name, batch, counter, output_file):
    ...
    for i in range(batch_size):
        ...
        for k in range(3,6):
            ...
            scores = rouge.get_scores([batch.summary_str_batch[i]], [ext_summary.strip()])
...
class ExtGenDecoder(object):
  ...
  def __init__(self, model_ext, model_gen, batcher, word_vocab, mode="decode"):
    ...
    self.ext_score_1=np.zeros([10,3],dtype=np.float32).tolist()
    self.ext_score_l=np.zeros([10,3],dtype=np.float32).tolist()
    self.ext_score_2=np.zeros([10,3],dtype=np.float32).tolist()
    
    self.gen_score_1=np.zeros([3],dtype=np.float32).tolist()
    self.gen_score_l=np.zeros([3],dtype=np.float32).tolist()
    self.gen_score_2=np.zeros([3],dtype=np.float32).tolist()
    ...
    while True:
      ...
      if self._model_gen is not None:
          for index in range(FLAGS.batch_size):
            ...
            self.write_results_gen(batch.summary_str_batch[index], gen_output, counter*FLAGS.batch_size+index, gen_output_file)
    ...
    while True:
      ...
      for i in range(len(hyps)):
        ...
        try:
            ...
            decoded_words = decoded_words
        decoded_output = ' '.join(decoded_words) # single string
        self.write_results_gen_for_train(batch.summary_str_batch[i], decoded_output, all_summary_ext_greedy[i], counter*FLAGS.batch_size+i, trn_output_file)
  ...
  def write_results_ext(self, sent_scores, entity_scores, data_name, batch, counter, output_file):
    ...
    for i in range(batch_size):
        ...
        for k in range(3,6):
            ...
            scores = rouge.get_scores([batch.summary_str_batch[i]], [ext_summary.strip()])
            output_file.write("%d-%f\t%f\t%f\n" % (k, scores[0]['rouge-1']['r'], scores[0]['rouge-1']['p'], scores[0]['rouge-1']['f']))
            self.ext_score_1[k][0]+=float(scores[0]['rouge-1']['r'])
            self.ext_score_1[k][1]+=float(scores[0]['rouge-1']['p'])
            self.ext_score_1[k][2]+=float(scores[0]['rouge-1']['f'])
            output_file.write("%d-%f\t%f\t%f\n" % (k, self.ext_score_1[k][0]/self.num, self.ext_score_1[k][1]/self.num, self.ext_score_1[k][2]/self.num))
            
            output_file.write("%d-%f\t%f\t%f\n" % (k, scores[0]['rouge-l']['r'], scores[0]['rouge-l']['p'], scores[0]['rouge-l']['f']))
            self.ext_score_l[k][0]+=float(scores[0]['rouge-l']['r'])
            self.ext_score_l[k][1]+=float(scores[0]['rouge-l']['p'])
            self.ext_score_l[k][2]+=float(scores[0]['rouge-l']['f'])
            output_file.write("%d-%f\t%f\t%f\n" % (k, self.ext_score_l[k][0]/self.num, self.ext_score_l[k][1]/self.num, self.ext_score_l[k][2]/self.num))
            
            output_file.write("%d-%f\t%f\t%f\n" % (k, scores[0]['rouge-2']['r'], scores[0]['rouge-2']['p'], scores[0]['rouge-2']['f']))
            self.ext_score_2[k][0]+=float(scores[0]['rouge-2']['r'])
            self.ext_score_2[k][1]+=float(scores[0]['rouge-2']['p'])
            self.ext_score_2[k][2]+=float(scores[0]['rouge-2']['f'])
            output_file.write("%d-%f\t%f\t%f\n" % (k, self.ext_score_2[k][0]/self.num, self.ext_score_2[k][1]/self.num, self.ext_score_2[k][2]/self.num))


  def write_results_gen(self, ref, decoded_text, ex_index, output_file):
    target = ref.strip("\n")
    summary = decoded_text.strip("\n")
    ex_index += 1
    output_file.write("[%d]\n" % ex_index)
    ...
    try:
        ...
        scores = rouge.get_scores([ref], [decoded_text])
        output_file.write("%f\t%f\t%f\n" % (scores[0]['rouge-1']['r'], scores[0]['rouge-1']['p'], scores[0]['rouge-1']['f']))
        self.gen_score_1[0]+=float(scores[0]['rouge-1']['r'])
        self.gen_score_1[1]+=float(scores[0]['rouge-1']['p'])
        self.gen_score_1[2]+=float(scores[0]['rouge-1']['f'])
        output_file.write("%f\t%f\t%f\n" % (self.gen_score_1[0]/ex_index, self.gen_score_1[1]/ex_index, self.gen_score_1[2]/ex_index))
          
        output_file.write("%f\t%f\t%f\n" % (scores[0]['rouge-2']['r'], scores[0]['rouge-2']['p'], scores[0]['rouge-2']['f']))
        self.gen_score_2[0]+=float(scores[0]['rouge-2']['r'])
        self.gen_score_2[1]+=float(scores[0]['rouge-2']['p'])
        self.gen_score_2[2]+=float(scores[0]['rouge-2']['f'])
        output_file.write("%f\t%f\t%f\n" % (self.gen_score_2[0]/ex_index, self.gen_score_2[1]/ex_index, self.gen_score_2[2]/ex_index))
          
        output_file.write("%f\t%f\t%f\n" % (scores[0]['rouge-l']['r'], scores[0]['rouge-l']['p'], scores[0]['rouge-l']['f']))
        self.gen_score_l[0]+=float(scores[0]['rouge-l']['r'])
        self.gen_score_l[1]+=float(scores[0]['rouge-l']['p'])
        self.gen_score_l[2]+=float(scores[0]['rouge-l']['f'])
        output_file.write("%f\t%f\t%f\n" % (self.gen_score_l[0]/ex_index, self.gen_score_l[1]/ex_index, self.gen_score_l[2]/ex_index))
  ...
  def write_results_gen_for_train(self, ref, decoded_text, summary_ext, ex_index, output_file):
    target = ref.strip("\n")
    summary = decoded_text.strip("\n")
