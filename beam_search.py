...
def run_beam_search(sess, model, vocab, batch):
  ...
  enc_states, entity_states, entity_enc, dec_in_state = model.run_encoder(sess, batch)
...
def run_greedy_search(sess, model, vocab, batch):
  ...
  steps = 0
  while steps < FLAGS.max_dec_steps and finish < batch_size:
    latest_tokens = [h.latest_token for h in hyps] # latest token produced by each hypothesis
    latest_tokens = [t if t in xrange(vocab.size()) else vocab.word2id(data.UNKNOWN_TOKEN) for t in latest_tokens] # change any in-article temporary OOV ids to [UNK] id, so that we can lookup word embeddings
    states = [h.state for h in hyps] # list of current decoder states of the hypotheses
    prev_coverage = [h.coverage for h in hyps] # list of coverage vectors (or None)
    ...
    # Run one step of the decoder to get the new info
    (topk_ids, _, new_states, attn_dists, p_gens, new_coverage) = model.decode_onestep(sess=sess,
                        batch=batch,
                        latest_tokens=latest_tokens,
                        enc_states=enc_states,
                        entity_states=entity_states,
    ...
    for i in xrange(batch_size):
      ...
      new_hyp = h.extend(token=topk_ids[i],
                           log_prob=None,
                           state=new_state,
                           attn_dist=attn_dist,
                           p_gen=p_gen,
    ...
    for i in xrange(len(all_hyps)):
      h = all_hyps[i]
      #print(h.latest_token,vocab.word2id(data.STOP_DECODING))
      #print(h.latest_token)
      if h.latest_token == vocab.word2id(data.STOP_DECODING): # if stop token is reached...
        # If this hypothesis is sufficiently long, put in results. Otherwise discard.
        if steps >= FLAGS.min_dec_steps and lens[i] == 0:
          lens[i] = steps + 1
          finish += 1
          #print(lens[i])
  ...
  for i in range(len(lens)):
      if lens[i] == 0: lens[i] = FLAGS.max_dec_steps
  #print(lens)
  return all_hyps, lens


def run_greedy_search_2(sess, model, vocab, batch):
  # Run the encoder to get the encoder hidden states and decoder initial state
  enc_states, entity_states, entity_enc, dec_in_state = model.run_encoder(sess, batch)
  ...
  steps = 0
  decoded_ids = []
  latest_tokens = [vocab.word2id(data.START_DECODING)] * batch_size
  states = [dec_in_state] * batch_size
  prev_coverage = [np.zeros([batch.enc_batch.shape[1]])] * batch_size
  while steps < FLAGS.max_dec_steps:
    ...
    states = new_states
    latest_tokens = [t if t in xrange(vocab.size()) else vocab.word2id(data.UNKNOWN_TOKEN) for t in topk_ids]
    prev_coverage = new_coverage
    
    decoded_ids.append(topk_ids)
