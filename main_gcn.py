...
def train(mode="W.O.RL", gcn_level=2, rl_lambda=0.3, batch_size=10, gpu=1, corpus="dailymail", en_vocab_size=4, 
          use_entity_embedding="10", lambda_ee_train=0.0, use_gcn_entity="True", en_loss_ver="new", gcn_method="default"):
    ...
    if en_vocab_size==4:
        ...
        exp_name = "%s-20-gcn%d" % (mode, gcn_level)
    elif en_vocab_size==30:
        entity_vocab_path=common.path_entity_vocab_30
        entity_embedding_path=common.path_entity_vocab_embeddings_30
        exp_name = "%s-30-gcn%d" % (mode, gcn_level)
    elif en_vocab_size==50:
        entity_vocab_path=common.path_entity_vocab_50
        entity_embedding_path=common.path_entity_vocab_embeddings_50
        exp_name = "%s-50-gcn%d" % (mode, gcn_level)
    ...
    ''' % (gpu, data_path, exp_name, gcn_level, rl_lambda, batch_size, ext_use_entity_embedding, gen_use_entity_embedding,
           ...
           lambda_ee_train, use_gcn_entity, gcn_method, corpus))


def ext_greedy(ext_mode="W.O.RL", gcn_level=2, rl_lambda=0.3, batch_size=10, gpu=1, corpus="dailymail", is_batch=False, 
               en_vocab_size=4, use_entity_embedding="True", gcn_method="default"):
    ...
    while True and sign: 
        ...
        os.system('''
            ...
            python -m run_gcn \
                  ...
                  --corpus=%s 
        ''' % (gpu, data_path, exp_name, gcn_level, rl_lambda, batch_size, ext_use_entity_embedding, gen_use_entity_embedding, 
               entity_vocab_path, entity_embedding_path, gen_exp_name, entity_vocab_size, gcn_method, corpus))
...
def ext_gen_decode(ext_mode="W.O.RL", gcn_level=2, rl_lambda=0.3, batch_size=10, gpu=1, corpus="dailymail", is_batch=False, write_mode="decode", 
                   en_vocab_size=4, decode_gen=True, use_entity_embedding="True", lambda_ee_train=0.0, use_gcn_entity="True", en_loss_ver="new", 
                   gcn_method="default"):
    ...
    if en_vocab_size==4:
        ...
        entity_vocab_path=common.path_entity_vocab_30
        entity_embedding_path=common.path_entity_vocab_embeddings_30
        exp_name = "%s-30-gcn%d" % (ext_mode, gcn_level)
    elif en_vocab_size==50:
        entity_vocab_path=common.path_entity_vocab_50
        entity_embedding_path=common.path_entity_vocab_embeddings_50
        exp_name = "%s-50-gcn%d" % (ext_mode, gcn_level)
    ...
    data_path=common.path_chunked+"/test/*"
    path_ckpt = common.path_log_root_gcn + "/" + exp_name + "/train/checkpoint"
    ...
    while True and sign: 
        ...
        os.system('''
            ...
            python -m run_gcn \
                  ...
                  --corpus=%s 
        ''' % (gpu, data_path, exp_name, gcn_level, rl_lambda, batch_size, ext_use_entity_embedding, gen_use_entity_embedding,
                write_mode, entity_vocab_path, entity_embedding_path, gen_exp_name, entity_vocab_size, decode_gen, lambda_ee_train, use_gcn_entity,
                gcn_method, corpus))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--en_loss_ver', choices=['new', 'old'], default='new')
    parser.add_argument('--gcn_method', choices=['default', 'sota'], default='default')
    ...
    if args.action == 'train':
        train(mode=args.mode, gcn_level=args.gcn_level, rl_lambda=args.rl_lambda, batch_size=args.batch_size, gpu=args.gpu, corpus=args.corpus, 
              ...
              en_loss_ver=args.en_loss_ver, gcn_method=args.gcn_method)
        ...
        ext_gen_decode(ext_mode=args.mode, gcn_level=args.gcn_level, rl_lambda=args.rl_lambda, batch_size=args.batch_size, gpu=args.gpu, corpus=args.corpus, 
                       ...
                       lambda_ee_train=args.lambda_ee_train, use_gcn_entity=args.use_gcn_entity, en_loss_ver=args.en_loss_ver, gcn_method=args.gcn_method)
