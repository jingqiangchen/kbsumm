...
class NYTDoc:
        ...
        for child in node:
            if child.tag == 'body.head':
                for gchild in child:
                    ...
                    elif gchild.tag == 'byline':
                        byline = gchild.text #self.read_block(gchild)
                        #print(byline)
                        if byline is not None and len(byline) > 0:
                            if "byline" not in self.summaries:
                                self.summaries['byline'] = []
                            self.summaries['byline'].append(byline)
    ...
    def extract_text(file):
        ...
        for line in file:
            try:
                decoded = line#line.decode('utf-8', 'strict')
                #print(line)
