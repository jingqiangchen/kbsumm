...
from decode3 import BeamSearchDecoder, BeamSearchDecoder_RL, GreedyDecoder 
import util
...
tf.app.flags.DEFINE_string('log_root', common.path_log_root, 'Root directory for all logging.')
tf.app.flags.DEFINE_string('exp_name', 'TEO', 'Name for experiment. Logs will be saved in a directory with this name, under log_root.')
...
tf.app.flags.DEFINE_integer('entity_vocab_size', 40001, 'Size of entity vocabulary.')
...
tf.app.flags.DEFINE_integer('ckpt_max_to_keep', 80, ' ')
tf.app.flags.DEFINE_integer('ckpt_save_model_secs', 9000, ' ')
  ...
  with sess_context_manager as sess:
    ...
    while True: # repeats until interrupted
      ...
      if not np.isfinite(loss):
        #print("entity_enc", results["entity_enc"])
        #print("enc_states", results["enc_states"])
        #print("emb_enc_inputs", results["emb_enc_inputs"])
        #print("enc_lens", results["enc_lens"])
        #print("mention_outputs", results["mention_outputs"])
...
def run_eval(model, batcher, word_vocab, entity_vocab):
...
def decode_rl():
  ...
  hparam_list = ['mode', 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 
                 ...
                 'batch_size', 'max_dec_steps', 'max_enc_steps', 'coverage', 'cov_loss_wt', 
                 "use_entity", "use_mention_occurs", "use_entity_embedding", "pointer_gen", "corpus"]
  hps_dict = {}
  ...
  if hps.mode == 'train':
    ...
    model = SummarizationModel(hps, word_vocab, entity_vocab)
    ...
    decoder = BeamSearchDecoder_RL(model, batcher, word_vocab, entity_vocab)
...
def main(unused_argv):
  ...
  # Reason: in decode mode, we decode one example at a time.
  # On each step, we have beam_size-many hypotheses in the beam, so we need to make a batch of these hypotheses.
  if FLAGS.mode == 'decode' and FLAGS.dec_method=="beam-search":
    FLAGS.batch_size = FLAGS.beam_size
  ...
  if hps.mode == 'train':
    ...
    if FLAGS.dec_method=="beam-search":
        decoder = BeamSearchDecoder(model, batcher, word_vocab, entity_vocab)
    elif FLAGS.dec_method=="greedy":
        decoder = GreedyDecoder(model, batcher, word_vocab, entity_vocab)
    decoder.decode() # decode indefinitely (unless single_pass=True, in which case deocde the dataset exactly once)
  else:
    raise ValueError("The 'mode' flag must be one of train/eval/decode")
